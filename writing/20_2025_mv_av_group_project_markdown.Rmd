...
#join the tables
koppen_legit <- koppen_from_kgc %>% left_join(us_can_koppen_lookup,
...
accuracy_plot_data <- b %>% 
  group_by(`Climate Description`, name) %>% 
  ...
  arrange(`Climate Description`, desc(adj_accuracy)) %>% 
  ungroup()
...
#                names_to = "prognosticator and climate parameter",
...
#   filter(`prognosticator and climate parameter` == "consistency")
#
# df_accuracy <- pl_accuracy_plot_data %>%
#   filter(`prognosticator and climate parameter` == "accuracy")
#
# df_adj_accuracy <- pl_accuracy_plot_data %>%
#   filter(`prognosticator and climate parameter` == "adj_accuracy")
#
# # Create the plot
# ggplot() +
#   geom_col(data = df_consistency,
#            aes(x = name, y = coefficients, fill = `prognosticator and climate parameter`),
...
#            aes(x = name, y = coefficients, fill = `prognosticator and climate parameter`),
#            position = position_nudge(x = 0.2), width = 0.35) +
#   facet_wrap(~ `Climate Description`, scales = "free_x") +
#   labs(
