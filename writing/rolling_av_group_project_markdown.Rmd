...
#create timeseries. The above has 1270 rows. There should be 43 for 43 years. The consolidate call
#will squash everything into one row per year. There are NaNs (not a number) instead of NAs now,
#which are an artifact of the calculation of mean. We can replace them with NA later if the html
#widget will not allow. The widget does allow for NA.

foo_consolidated <- wide_rolling_avgs_year_only %>%
  group_by(year) %>%
  summarise(across(everything(), mean, na.rm = TRUE))

#This looks right. Can we make a timeseries now?

library(xts)
library(zoo)

#year column must be in a date format
foo_consolidated$year <- ymd(paste0(foo_consolidated$year, "-02-02"))

#convert to xts class, which is friendly to dygraphs
foo_consolidated_ts <- as.xts(foo_consolidated)

#try DYGRAPHS to see if it works!
dygraph(foo_consolidated_ts, main = "5 yr Rolling Avgs")
#it works, but too busy.
```

# try Plotlywith a pulldown
# below works, but only able to plot one prognosticator at a time

```{r}
library(plotly)
library(dplyr)

#having hyphens in the name column is a problem
names(foo_consolidated) <- gsub("-", "_", names(foo_consolidated))

#get rid of data column and generate vectors with just names and dates
foo_consolidated_names <- names(foo_consolidated)[-1]
foo_consolidated_dates <- names(foo_consolidated)[1]

#base plot
p_fc <- plot_ly(foo_consolidated,
                x = as.formula(paste0("~", foo_consolidated_dates)))

#add in the data columns
for(col in  foo_consolidated_names) {
  p_fc <- p_fc %>% 
    add_lines(y = as.formula(paste0("~",
                                    col)), name = col, visible = TRUE)
}

button_list <- list(
  list(method = "restyle",
       args = list("visible", as.list(rep(TRUE,
                                          length(foo_consolidated_names)))),
       label = "ALl Series")
)

for(i in seq_along(foo_consolidated_names)) {
  visible_vector <- rep(FALSE, length(foo_consolidated_names))
  visible_vector[i] <- TRUE
  
  button_list[[i + 1]] <- list(
    method = "restyle",
    args = list("visible", as.list(visible_vector)),
    label = foo_consolidated_names[i]
  )
}
...
p_fc <- p_fc %>%
  layout(
    title = "5 year rolling average for proportion correct",
...
p_fc
```
Well, shiiiiittttttt. The above was cool, but below I will rearrange the
foo_consolidated dataframe into tidy format, make a ggplot, and then
try to get ggplotly to work so I can incorporate gghighlight
```{r plotly attempt}
library(gghighlight)

#pivot longer for tidyness; change NaNs to NAs just in case
foo_consolidated_tidy <- foo_consolidated %>% 
  pivot_longer(cols = -1, names_to = "prognosticator", values_to = "rolling_avg_5" )
foo_consolidated_tidy <- foo_consolidated_tidy %>% 
  mutate(across(3, ~na_if(., NaN)))

ggplot(foo_consolidated_tidy) +
  geom_line(aes(x = year, y = rolling_avg_5, colour = prognosticator))
#meh

# #BELOW ALSO SUCKS, SO COMMENTING OUT FOR NOW

# # Create plotly figure with custom highlighting
# fig <- plot_ly()
# 
# # Add each category as a separate trace
# for(cat in unique(foo_consolidated_tidy$prognosticator)) {
#   cat_data <- foo_consolidated_tidy %>% filter(prognosticator == cat)
#   
#   fig <- fig %>%
#     add_trace(
#       data = cat_data,
...
# }
# 
# # Update layout with dropdown menu for highlighting
# fig <- fig %>%
#   layout(
#     title = "5 yr rolling Avg (use legend to toggle)",
