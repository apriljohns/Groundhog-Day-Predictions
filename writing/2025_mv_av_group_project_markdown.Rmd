...
#join the tables
koppen_from_kgc %>% left_join(us_can_koppen_lookup,
...
accuracy_plot_data <- b %>% 
  ...
  summarise(accuracy = sum(is_correct, na.rm = TRUE)/n(),
            local_consistency = sum(six_more_weeks/n())) %>% 
  ungroup()

#try plotly for grouped bar chart

library(plotly)

accuracy_plot <- plot_ly(accuracy_plot_data,
                         x = ~`Climate Description`,
                         y = ~accuracy,
                         type = 'bar',
                         name = 'accuracy')
accuracy_plot <- accuracy_plot %>% 
  add_trace(y = ~local_consistency, name = 'local consistency')
accuracy_plot <- accuracy_plot %>% 
  layout(yaxis = list(title = 'Proportion correct or consistent'),
         barmode = 'group')

#This is not showing the individual pronosticators within each Koppen zone
#nor is it showing the scale of the y-axis correctly: fix on 12/8.
