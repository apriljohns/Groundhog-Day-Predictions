...
consist_plot <- foo %>% ggplot(aes(x = consistency, y = accuracy, colour = name)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, colour = "black", linewidth = .3) +
  theme(legend.position = "none") +
  annotate("text", x = .13, y = .36, label = "italic(R)^2 == .035",
           parse = TRUE, size = 3, angle = 10.5) +
  labs(x = "Early Spring Consistency\n at Prognoticator Location",
       y = "Pronosticator Accuracy") +
  theme(axis.title.x = element_text(size = 8, vjust = 3),
